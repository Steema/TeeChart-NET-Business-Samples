...
using WebAppNET60.Models;

namespace WebAppNET60.Controllers
{
    public class HomeController : Controller
    {
        ...
        public void GetChartImage()
        {
            lock (s_renderLock)
            {
                var wChart = new Steema.TeeChart.TChart();

                var xpTheme = new Steema.TeeChart.Themes.AndrosTheme(wChart.Chart);
                xpTheme.Apply();
                ...
                wChart.Export.Image.PNG.Save(_tempStream1);
                _tempStream1.Flush();
        ...
        public void GetHTML5Chart(int? type)
        {
            ...
            {
                {
                    var wChart3 = new Steema.TeeChart.TChart();
        ...
        public void GetPieChartx()
        {
            lock (s_renderLock)
            {
                var wChart2 = new Steema.TeeChart.TChart();
        ...
        public ActionResult GetEqualizerChart(int? w, int? h)
        {
            lock (s_renderLock)
            {
                ...
                var rpTheme = new Steema.TeeChart.Themes.BlackIsBackTheme(wChart2.Chart);
                rpTheme.Apply();
